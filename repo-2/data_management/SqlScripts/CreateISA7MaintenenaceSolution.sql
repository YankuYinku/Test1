...
-- create a job category named AlwaysOn, to allow failover of SQL jobs  
-- create agent job "DBA - Check Jobs Failover", to monitor for failover of AG and then fail jobs over
-- create Hallengren maintenance objects
-- create agent job "DBA - DatabaseIntegrityCheck - SYSTEM_DATABASES"
-- create agent job "DBA - DatabaseIntegrityCheck - USER_DATABASES"
-- create job "DBA - Reindex"
-- create database DBAData, including Audit spec objects
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Check Jobs Failover', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'check if this server is primary and enable or disable sql jobs if relevant', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [run sql]    Script Date: 23/11/2018 10:15:52 ******/
...
-- check which node is primary
 ...
 DECLARE @is_primary_replicate int
...
-- set the variable @is_primary_replicate

  IF UPPER(@PrimaryReplica) =  UPPER(@@SERVERNAME)
  set @is_primary_replicate = 1
  else   set @is_primary_replicate = 0


 -- Get a list of ALL the jobs in the category "AlwaysOn"

   declare job_cursor cursor for 
   select s.name from msdb.dbo.sysjobs s
   inner join msdb.dbo.syscategories c on s.category_id = c.category_id
   where c.name = ''AlwaysOn''
   order by name

   -- FOR EACH job in the category "AlwaysOn"
   ...
   begin
		  ...
		  -- get the current job status (enabled or not)

		  select @job_enabled=
		  -- select
		  enabled from msdb.dbo.sysjobs where name = @job_name
          
		  -- if primary server - enable any jobs which arent already enabled
		  ...
		  -- if NOT primary server - disable any jobs which arent already disabled
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'frequent', 
		...
		@schedule_uid=N'cc34c96a-d8d7-4ce8-8a9c-9f314ca295b6'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  Job [DBA - DatabaseIntegrityCheck - SYSTEM_DATABASES]    Script Date: 10/25/2022 9:25:27 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 10/25/2022 9:25:27 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - DatabaseIntegrityCheck - SYSTEM_DATABASES', 
		...
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DBA - DatabaseIntegrityCheck - SYSTEM_DATABASES]    Script Date: 10/25/2022 9:25:27 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DBA - DatabaseIntegrityCheck - SYSTEM_DATABASES', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''SYSTEM_DATABASES'',
  @ExtendedLogicalChecks = ''Y'', @LogToTable = ''Y''', 
		@database_name=N'master', 
		@output_file_name=N'D:\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQL\Log\DatabaseBackup_$(ESCAPE_SQUOTE(JOBID))_$(ESCAPE_SQUOTE(STEPID))_$(ESCAPE_SQUOTE(STRTDT))_$(ESCAPE_SQUOTE(STRTTM)).txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA - IntegrityCheck - System Databases', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@active_end_time=235959, 
		@schedule_uid=N'cdde2a9b-e974-4102-b7ba-c854626bd5ff'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [DBA - DatabaseIntegrityCheck - USER_DATABASES]    Script Date: 10/25/2022 9:25:32 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [AlwaysOn]    Script Date: 10/25/2022 9:25:32 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - DatabaseIntegrityCheck - USER_DATABASES', 
		...
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [DBA - IntegrityCheck - User Databases]    Script Date: 10/25/2022 9:25:32 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DBA - IntegrityCheck - User Databases', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''USER_DATABASES'',
  @CheckCommands = ''checkdb'', @LogToTable = ''Y''', 
		@database_name=N'master', 
		@output_file_name=N'D:\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQL\Log\DatabaseBackup_$(ESCAPE_SQUOTE(JOBID))_$(ESCAPE_SQUOTE(STEPID))_$(ESCAPE_SQUOTE(STRTDT))_$(ESCAPE_SQUOTE(STRTTM)).txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'DBA - IntegrityCheck - User Databases', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@active_end_time=235959, 
		@schedule_uid=N'aca30a70-0b34-439b-80b9-6e91096d81b3'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [DBA - Reindex]    Script Date: 10/25/2022 9:31:45 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [AlwaysOn]    Script Date: 10/25/2022 9:31:45 AM ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'AlwaysOn' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'AlwaysOn'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Reindex', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Call maintenance stored procedure provided by the vendor', 
		@category_name=N'AlwaysOn', 
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Call stored procedure]    Script Date: 10/25/2022 9:31:45 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Call stored procedure', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'/*                        Script Details                           *
 *-----------------------------------------------------------------*

    Project             : ISA Data Purge
    Filename            : Execute_DatabaseMaintenance.sql

 *-----------------------------------------------------------------*
 *                          Description                            *
 *-----------------------------------------------------------------*

  Script to be copied into the desired scheduling system to enable execution of the DataBaseMaintainance.
  Notes:
  Copy the whole of this script into the scheduling tool

 *-----------------------------------------------------------------*
 *          (c) Copyright 2021 Cash Processing Solutions           *
 *-----------------------------------------------------------------*
...
BEGIN TRY
  ...
  EXEC @errCode = MAESTRO_IDP.SPIDP_DATABASEMAINTENANCEPROCEDURE
    @errMsg = @errMessage OUTPUT;
END TRY
BEGIN CATCH
...
IF @errCode <> 0
  BEGIN
  PRINT 
    CAST(@errCode AS VARCHAR(MAX))+'', ''
    +CAST(@errMessage AS VARCHAR(MAX));
  RAISERROR(@errMessage ,16,1);
  END
GO', 
		@database_name=N'ISADB', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Weekly', 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=1, 
		@active_start_date=20221025, 
		@active_end_date=99991231, 
		@active_start_time=0, 
		@active_end_time=235959, 
		@schedule_uid=N'8b1d9fa9-a566-484f-af5b-f81cf5c5f936'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Database [DBAData]    Script Date: 18/10/2018 14:00:27 ******/
CREATE DATABASE [DBAData]
-- ON  PRIMARY 
--( NAME = N'DBAData', FILENAME = N'D:\DATA\DBAData.mdf' , SIZE = 524288KB , MAXSIZE = UNLIMITED, FILEGROWTH = 524288KB )
-- LOG ON 
--( NAME = N'DBAData_log', FILENAME = N'L:\LOGS\DBAData_log.ldf' , SIZE = 524288KB , MAXSIZE = 2048GB , FILEGROWTH = 524288KB )
GO


--IF (1 = FULLTEXTSERVICEPROPERTY('IsFullTextInstalled'))
--begin
--EXEC [DBAData].[dbo].[sp_fulltext_database] @action = '
--'
--end
--GO
...
/****** Object:  Table [dbo].[DDLEvents]    Script Date: 18/10/2018 14:01:03 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  Table [dbo].[AuditSpecification_ServiceAccountsToIgnore]    Script Date: 07/11/2018 10:19:34 ******/
SET ANSI_NULLS ON
GO
...
CREATE TABLE [dbo].[AuditSpecification_ServiceAccountsToIgnore](
	[ServiceAccountName] [VARCHAR](100) NOT NULL,
...
/****** Object:  StoredProcedure [dbo].[get_AuditSpecificationEvents]    Script Date: 07/11/2018 10:18:41 ******/
SET ANSI_NULLS ON
GO
...
create PROCEDURE [dbo].[get_AuditSpecificationEvents]

AS

/*

get an Audit log events
that occurred in the past 7 days 
and werent executed by the sql or sql agent accounts
or executed any accounts listed in the table DBAData.[dbo].[AuditSpecification_ServiceAccountsToIgnore]

*/

-- CHECK if ServiceAccountsToIgnore have been specified

IF NOT EXISTS
(
SELECT * FROM DBAData.[dbo].[AuditSpecification_ServiceAccountsToIgnore]
)
raiserror('ERROR, You havent defined any service accounts to be omitted from these audits

Please insert rows into table DBAData.[dbo].[AuditSpecification_ServiceAccountsToIgnore]', 10, -1)  -- If there are NO rows in ServiceAccountsToIgnore, then ERRROR

ELSE 


BEGIN -- Otherwise Continue to Query the audits

SELECT 
	@@servername AS [ServerName], 
SERVERPROPERTY('MachineName') AS [HostName],
@@servicename AS [Instance],
 CONVERT(DATETIME, event_time,120) AS event_time
	,action_id,
	CASE action_id
...
,schema_name
,object_name
 ,left(statement,4000) as [statement] 
 -- select * 
FROM sys.fn_get_audit_file('c:\temp\auditspecification\logs\*.sqlaudit', DEFAULT, DEFAULT)


-- ignore records from service accounts specified in ServiceAccountsToIgnore
WHERE session_server_principal_name NOT IN (SELECT serviceaccountname FROM DBAData.[dbo].[AuditSpecification_ServiceAccountsToIgnore])

-- ignore records written by sql or sql sgent service accounts
AND session_server_principal_name NOT IN (SELECT DSS.service_account  FROM sys.dm_server_services AS DSS)

and event_time > GETDATE()-7 -- events from the past 7 days

AND OBJECT_NAME <> '##MS_SSISServerCleanupJobLogin##'
and statement not like '%sys.spt_columns_view%'
and statement not like 'Network error code%'
