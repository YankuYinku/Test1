...
{
    'devappsisac1'      { $keyvault = 'dev-apps-isa-uks-kv';    $endpoint = 'devappsisalb.mgt.prvdns.vaultexuk.net'      }
    ...
    'sitappsisac1'      { $keyvault = 'sit-apps-isa-uks-kv';    $endpoint = 'sitappsisalb.mgt.prvdns.vaultexuk.net'      }
}
...
# Replace the standard details in the Config files for the CIF Services with the Database Connection string from the Keyvault
#
# Core Service
...
# Standing Order Issue Interface
$config_file = "D:\Program Files\CPS\CPS Issue Standing Orders\CPS Issue Standing Orders.dll.config"
(Get-Content $config_file) | Foreach-Object {
    $_ -replace ([regex]::Escape('Persist Security Info=False;User ID=ISAUser;Password=n2r2SNPD0M3dbRUp+qasRA==;Database=ISADB;Server=(local)')), $CIFCIConnectionString.value `
} | Set-Content $config_file
...
#
# Update the server name of the Web Service Server in the files for the CIF Services
#
# Client Import
...
# Order Import
$config_file = "D:\Program Files\CPS\CPS Order Import Service\CPS.OrderImportWinService.exe.config"
(Get-Content $config_file) | Foreach-Object {
    $_ -replace ([regex]::Escape('endpoint address="http://localhost/Services/Wintran"')), "endpoint address=`"https://$endpoint/Services/Wintran`"" `
    -replace ([regex]::Escape('endpoint address="http://localhost/Services/Order"')), "endpoint address=`"https://$endpoint/Services/Order`"" `
    -replace ([regex]::Escape('<binding name="BasicHttpBinding_IOrderService" />')), "<binding name=`"BasicHttpBinding_IOrderService`">`r`n`t`t<security mode=`"Transport`">`r`n`t`t`t<transport clientCredentialType=`"None`" />`r`n`t`t</security>`r`n`t </binding> " `
    -replace ([regex]::Escape('<binding name="BasicHttpBinding_IWcsService" />')), "<binding name=`"BasicHttpBinding_IWcsService`">`r`n`t`t<security mode=`"Transport`">`r`n`t`t`t<transport clientCredentialType=`"None`" />`r`n`t`t</security>`r`n`t</binding> " `
} | Set-Content $config_file

# Standing Order Issue
$config_file = "D:\Program Files\CPS\CPS Issue Standing Orders\CPS.WinService.exe.config"
(Get-Content $config_file) | Foreach-Object {
    $_ -replace ([regex]::Escape('endpoint address="http://localhost/Services/Order"')), "endpoint address=`"https://$endpoint/Services/Order`"" `
} | Set-Content $config_file
